...
unless style?.props.children == \
       '@media (min-width: 0px) { #imported_tile\\.part[data-kind="{tile}"] > rect { fill: url(#imported_paint); } }'
  throw new Error 'Wrong imported <style> contents'

# Check raw vs. processed SVG strings.
unless imported.raw.includes('id="tile.part"') and
       imported.svg.includes('id="imported_tile.part"') and
